...
def load_settings(project_name):
    ...
    # Default settings
    default_settings = {
        "exp_name": "F5TTS_v1_Custom_Prune_12",
        "learning_rate": 1e-5,
...
# Make sure these are imported at the top of the file if not already
import torch
...
# Optional: Set up simple logging if not already done
# logging.basicConfig(level=logging.INFO)
# logger = logging.getLogger(__name__)
# Use print statements if logging is not set up

def expand_model_embeddings(ckpt_path, new_ckpt_path, num_new_tokens):
    """
    Expands the token embeddings in a checkpoint file.
    Handles both original EMA/non-EMA .pt/.safetensors and pruned .pt files.
    Saves the output checkpoint with the expanded embeddings, preserving relevant metadata.
    """
    if num_new_tokens <= 0:
        print("Warning: num_new_tokens is <= 0. No expansion needed. Copying original file.")
        # Check if paths are the same to avoid error
        if os.path.abspath(ckpt_path) != os.path.abspath(new_ckpt_path):
             shutil.copyfile(ckpt_path, new_ckpt_path)
             print(f"Copied original checkpoint to {new_ckpt_path}")
        else:
             print("Source and destination paths are the same. Skipping copy.")
        # Need to determine original vocab size for return value consistency
        try:
            temp_ckpt = torch.load(ckpt_path, map_location="cpu") if ckpt_path.endswith(".pt") else load_file(ckpt_path, device="cpu")
            # Simplified check for size, might need refinement based on structure
            if isinstance(temp_ckpt, dict):
                sd = temp_ckpt.get('model_state_dict', temp_ckpt.get('ema_model_state_dict', temp_ckpt))
                embed_key = next((k for k in sd if k.endswith('text_embed.text_embed.weight')), None)
                if embed_key: return sd[embed_key].shape[0]
            elif isinstance(temp_ckpt, dict) and ckpt_path.endswith(".safetensors"): # raw safetensor state_dict
                 embed_key = next((k for k in temp_ckpt if k.endswith('text_embed.text_embed.weight')), None)
                 if embed_key: return temp_ckpt[embed_key].shape[0]

        except Exception:
             print("Could not determine original vocab size for no-expansion case.")
        return -1 # Indicate no expansion or error

    # --- Load the Checkpoint ---
    print(f"Loading checkpoint for embedding expansion: {ckpt_path}")
    source_sd = None
    original_checkpoint_structure = {} # To store other keys like config, pruning_info
    is_ema_prefix_present = False # Flag to track if keys *within* the state dict have the prefix

    if ckpt_path.endswith(".safetensors"):
        # Safetensors contain only the state_dict
        source_sd = load_file(ckpt_path, device="cpu")
        # Check if keys inside have EMA prefix (unlikely for direct safetensors, but check)
        if source_sd and any(k.startswith("ema_model.") for k in source_sd):
             is_ema_prefix_present = True
        print(f"Loaded state_dict directly from safetensors. EMA prefix in keys: {is_ema_prefix_present}")
    elif ckpt_path.endswith(".pt"):
        original_checkpoint_structure = torch.load(ckpt_path, map_location="cpu")
        if not isinstance(original_checkpoint_structure, dict):
             raise ValueError(".pt file did not load as a dictionary.")

        # Find the state dict within the .pt file structure
        if 'model_state_dict' in original_checkpoint_structure:
            source_sd = original_checkpoint_structure['model_state_dict']
            print("Found state_dict under key: 'model_state_dict'")
            # Check for internal EMA prefix (should NOT be present for pruned file)
            if source_sd and any(k.startswith("ema_model.") for k in source_sd):
                 print("Warning: Found 'ema_model.' prefix inside 'model_state_dict'. This might be unexpected.")
                 is_ema_prefix_present = True # This case shouldn't happen with correct pruning
        elif 'ema_model_state_dict' in original_checkpoint_structure:
            source_sd = original_checkpoint_structure['ema_model_state_dict']
            print("Found state_dict under key: 'ema_model_state_dict'")
            # Expect EMA prefix inside this dict
            if source_sd and any(k.startswith("ema_model.") for k in source_sd):
                 is_ema_prefix_present = True
            else:
                 print("Warning: Did not find expected 'ema_model.' prefix inside 'ema_model_state_dict'.")
        # Add more fallbacks if needed (e.g., 'model', 'state_dict')
        else:
            raise KeyError(f"Could not find 'model_state_dict' or 'ema_model_state_dict' in {ckpt_path}")

    if source_sd is None:
        raise ValueError("Failed to load or find the state dictionary.")

    # --- Clean Keys (Strip Prefix if Present) ---
    cleaned_sd = OrderedDict()
    prefix_to_strip = "ema_model." if is_ema_prefix_present else None
    ignore_keys = {"initted", "step"} # Metadata keys to always ignore

    print(f"Processing state_dict keys. Stripping prefix: '{prefix_to_strip}'")
    for k, v in source_sd.items():
        # Ignore metadata keys
        key_to_check_ignore = k[len(prefix_to_strip):] if prefix_to_strip and k.startswith(prefix_to_strip) else k
        if key_to_check_ignore in ignore_keys:
            print(f"Ignoring metadata key: {k}")
            continue

        # Strip prefix if needed
        final_key = k
        if prefix_to_strip and k.startswith(prefix_to_strip):
            final_key = k[len(prefix_to_strip):]

        cleaned_sd[final_key] = v

    if not cleaned_sd:
         raise ValueError("State dictionary is empty after cleaning prefixes and metadata.")
    print(f"Cleaned state_dict has {len(cleaned_sd)} keys.")


    # --- Define CLEANED Key Names ---
    # These are the keys expected AFTER any prefix stripping
    embed_key = 'transformer.text_embed.text_embed.weight'
    bias_key = 'transformer.text_embed.text_embed.bias' # Optional bias
    # Output projection usually doesn't need resizing, but ensure it's handled
    proj_key = 'transformer.proj_out.weight'
    proj_bias_key = 'transformer.proj_out.bias' # Optional bias

    # Check if necessary keys exist in the cleaned state dict
    if embed_key not in cleaned_sd:
        print("Available keys:", list(cleaned_sd.keys())[:20]) # Print some keys for debugging
        raise KeyError(f"Cleaned embedding key '{embed_key}' not found in the processed state_dict!")
    # proj_key check might be too strict if model variants exist, maybe make optional?
    # if proj_key not in cleaned_sd:
    #    print("Available keys:", list(cleaned_sd.keys())[:20])
    #    raise KeyError(f"Cleaned projection key '{proj_key}' not found in the processed state_dict!")


    # --- Get Old Tensors ---
    print(f"Accessing weights using CLEANED keys (e.g., '{embed_key}')")
    old_embed = cleaned_sd[embed_key]
    old_proj = cleaned_sd.get(proj_key) # Use .get in case proj key differs slightly
    old_bias = cleaned_sd.get(bias_key)
    old_proj_bias = cleaned_sd.get(proj_bias_key)

    old_vocab_size, embed_dim = old_embed.shape
    vocab_new = old_vocab_size + num_new_tokens
    print(f"Old vocab size: {old_vocab_size}, Embed dim: {embed_dim}")
    print(f"New vocab size: {vocab_new}")

    # --- Expand Embeddings (and bias if present) ---
    # Initialize new embeddings randomly (could use mean/std for better init)
    new_embed = torch.zeros(vocab_new, embed_dim, dtype=old_embed.dtype)
    new_embed.normal_(0, 0.02) # Small random values for new tokens
    # Copy old weights
    new_embed[:old_vocab_size, :] = old_embed
    print(f"Created new embedding tensor with shape: {new_embed.shape}")

    new_bias = None
    if old_bias is not None:
        new_bias = torch.zeros(vocab_new, dtype=old_bias.dtype)
        new_bias[:old_vocab_size] = old_bias
        # Initialize new bias (e.g., to zero)
        new_bias[old_vocab_size:] = 0.0
        print(f"Created new bias tensor with shape: {new_bias.shape}")


    # --- Create the NEW Expanded State Dictionary ---
    # Use OrderedDict to preserve key order
    new_expanded_sd = OrderedDict()
    for k, v in cleaned_sd.items():
        if k == embed_key:
            new_expanded_sd[k] = new_embed
        elif k == bias_key and new_bias is not None:
            new_expanded_sd[k] = new_bias
        # Projection layers and other layers are copied directly
        else:
            new_expanded_sd[k] = v

    # Ensure bias key is present if new_bias was created but old_bias was None (edge case)
    if old_bias is None and new_bias is not None and bias_key not in new_expanded_sd:
        print(f"Adding newly created bias tensor under key '{bias_key}'")
        new_expanded_sd[bias_key] = new_bias
        # Reorder if desired (e.g., put bias right after weight)
        # new_expanded_sd.move_to_end(bias_key, last=False)
        # embed_items = list(new_expanded_sd.items())
        # try:
        #     weight_idx = list(new_expanded_sd.keys()).index(embed_key)
        #     bias_idx = list(new_expanded_sd.keys()).index(bias_key)
        #     if bias_idx != weight_idx + 1:
        #         # Simple re-insert, might change order slightly elsewhere
        #         del new_expanded_sd[bias_key]
        #         items = list(new_expanded_sd.items())
        #         items.insert(weight_idx + 1, (bias_key, new_bias))
        #         new_expanded_sd = OrderedDict(items)
        # except ValueError: pass # Should not happen


    # --- Prepare the Final Checkpoint Structure for Saving ---
    final_output_checkpoint = {}
    # *** Crucially, save the expanded state dict under 'model_state_dict' ***
    final_output_checkpoint['model_state_dict'] = new_expanded_sd

    # Copy other relevant keys from the original loaded structure (if .pt)
    # Avoid copying the original state dict key itself
    original_sd_key = None
    if 'model_state_dict' in original_checkpoint_structure: original_sd_key = 'model_state_dict'
    elif 'ema_model_state_dict' in original_checkpoint_structure: original_sd_key = 'ema_model_state_dict'
    # Add more checks if needed

    for key, value in original_checkpoint_structure.items():
        if key != original_sd_key and key not in final_output_checkpoint:
             # Copy keys like 'config', 'hyper_parameters', 'pruning_info', etc.
             final_output_checkpoint[key] = value
             print(f"Copied key '{key}' from original checkpoint.")


    # --- Save the New Checkpoint ---
    print(f"Saving checkpoint with expanded embeddings to: {new_ckpt_path}")
    if new_ckpt_path.endswith(".safetensors"):
        # Save only the state dict for safetensors, potentially add metadata
        save_metadata = {"embeddings_expanded": "true", "original_vocab": str(old_vocab_size), "new_vocab": str(vocab_new)}
        if 'pruning_info' in final_output_checkpoint:
             try: # Add some pruning info if possible
                  save_metadata["pruning_original_layers"] = str(final_output_checkpoint['pruning_info'].get('original_transformer_indices'))
                  save_metadata["pruning_final_layers"] = str(final_output_checkpoint['pruning_info'].get('final_transformer_layers'))
             except Exception: pass # Ignore errors converting pruning info

        save_file(final_output_checkpoint['model_state_dict'], new_ckpt_path, metadata=save_metadata)
        print("Saved as .safetensors")
    elif new_ckpt_path.endswith(".pt"):
        # Save the full dictionary structure for .pt
        torch.save(final_output_checkpoint, new_ckpt_path)
        print("Saved as .pt")
    else:
        print(f"Warning: Unknown output file extension for {new_ckpt_path}. Saving in .pt format.")
        torch.save(final_output_checkpoint, new_ckpt_path)

    print("Expansion and save complete.")
    return vocab_new
...
def vocab_extend(project_name, symbols, model_type):
    ...
    new_ckpt_file = os.path.join(new_ckpt_path, os.path.basename(ckpt_path))
...
```""")
            with gr.Row():
                exp_name = gr.Radio(label="Model", choices=["F5TTS_v1_Custom_Prune_14", "F5TTS_v1_Custom_Prune_12", "F5TTS_v1_Base", "F5TTS_Base", "E2TTS_Base"])
